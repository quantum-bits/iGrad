...
# I get an error saying that the "ManyRelatedManager object is not iterable"
    ...
    for ssc in tempdata:
        for course in ssc.courses.all():
            sem=ssc.semester
            studentcourselist.append([course.name, course.number, sem])
    ...
    for mr in studentmajor.major_requirements.all():
        ...
        for course in mr.courselist.all():
            ...
            if coursetaken!=[]:
                total_credit_hours_so_far+=course.credit_hours
        ...
        majordatablock.append([mr.list_order, mr.name, AND_OR_comment, 
                                   ...
                                   mr.text_for_user, courselisttemp, total_credit_hours_so_far, credits_ok])
