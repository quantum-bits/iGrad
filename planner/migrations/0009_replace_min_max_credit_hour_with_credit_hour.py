...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for course in orm.Course.objects.all():
            if course.min_credit_hours == course.max_credit_hours:
                name = str(course.min_credit_hours)
                credit_hour, created = orm.CreditHour.objects.get_or_create(name=name, value=name)
                course.credit_hour = credit_hour
            else:
                name = "{} to {}".format(course.min_credit_hours, course.max_credit_hours)
                value = name.replace('to', '-')
                credit_hour, created = orm.CreditHour.objects.get_or_create(name=name, value=value)
                course.credit_hour = credit_hour
            course.save()
    ...
    def backwards(self, orm):
        ...
        for course in orm.Course.objects.all():
            credit_hours = course.credit_hour.possible_credit_hours() 
            if len(credit_hours) == 1:
                course.min_credit_hours = course.max_credit_hours = credit_hours[0]
            else:
                course.min_credit_hours = credit_hours[0]
                course.max_credit_hours = credit_hours[-1]
